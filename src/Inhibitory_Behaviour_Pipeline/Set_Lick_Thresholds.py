...
class lick_threshold_window(QWidget):
    ...
    def __init__(self, data_directory_root, session_directory_list, parent=None):
        ...
        self.channel_dictionary = Behaviour_Analysis_Utils.create_daq_channel_dict()
    ...
    def load_all_data(self):
        ...
        for session_index in tqdm(range(number_of_sessions)):
            ...
            # Load Lick Trace
            ai_data = Behaviour_Analysis_Utils.load_ai_recorder_data(os.path.join(self.data_directory_root, current_session))
            ...
            if os.path.exists(os.path.join(self.data_directory_root, current_session, "Lick_Threshold.npy")):
                lick_threshold = np.load(os.path.join(self.data_directory_root, current_session, "Lick_Threshold.npy"))
    ...
    def set_lick_threshold(self):
         ...
         # Get Output Path
        file_save_directory = os.path.join(self.data_directory_root, self.session_directory_list[self.current_session_index], "Lick_Threshold.npy")
...
def set_lick_thresholds(data_root, session_list):

    app = QApplication(sys.argv)
    window = lick_threshold_window(data_root, session_list)
    window.show()
